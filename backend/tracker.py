...
def get_specific_app_name(title, app_name):
    ...
    if app_lower in ["chrome.exe", "msedge.exe", "firefox.exe", "brave.exe", "opera.exe", "applicationframehost.exe"]:
        # It's a browser. Check if any distraction keyword matches
        for keyword in app_state.distraction_keywords:
            if keyword.lower() in title.lower():
                return keyword
                
        # If no distraction keyword, try to extract the website title
        if " - " in title:
            parts = title.split(" - ")
            site_name = " - ".join(parts[:-1]).strip()
            if site_name:
                return site_name
        return title
            ...
            if title and title != "Unknown":
                ...
                try:
                    ...
                    if is_distraction:
                        ...
                        activity_classifier.train(title, "Work")
                except Exception as e:
